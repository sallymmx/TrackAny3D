...
class BaseTask(pl.LightningModule):
    ...
    def _on_test_epoch_start_kitti_format(self):
        ...
        self.runtime.reset()
        self.n_frames.reset()
        if self.cfg.model_cfg.model_type == 'TrackAny':
            with torch.no_grad():
                for _ in range(100):
                    backbone_input = dict(
                        pcds=torch.randn(1, 2, 1024, 3).cuda(),
                        mask_refs = torch.randn(1,2,1024).cuda(),
                        cls_token = None,
                        bc_refs = torch.randn(1,2,1024,9).cuda()
                    )
    ...
    def _test_step_kitti_format(self, batch, batch_idx):
        ...
        self.log('precesion', self.prec.compute(), prog_bar=True, logger=False, batch_size=n_frames)
        self.log('success', self.succ.compute(), prog_bar=True, logger=False, batch_size=n_frames)
        self.log('n_frames', self.n_frames.compute(),
                 prog_bar=True, logger=False, batch_size=n_frames)
